...
#define CCM_RST_OFFSET (16)
#define CCM_GATING_OFFSET (0)

static void hal_pwm_clock_enable() {
    // enable clock
    SET_BIT(CCU->PWM_BGR_REG, CCM_GATING_OFFSET);

    // deassert reset
    SET_BIT(CCU->PWM_BGR_REG, CCM_RST_OFFSET);
}

static void hal_uart_clock_enable(uint32_t port) {
    // enable clock
    SET_BIT(CCU->UART_BGR_REG, CCM_GATING_OFFSET + port);

    // deassert reset
    SET_BIT(CCU->UART_BGR_REG, CCM_RST_OFFSET + port);
}
